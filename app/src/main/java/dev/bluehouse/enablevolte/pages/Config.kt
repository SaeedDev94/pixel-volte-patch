...
fun Config(navController: NavController, subId: Int) {
    ...
    if (loading) {
        ...
        Column(modifier = Modifier.padding(Dp(16f)).verticalScroll(scrollState)) {
            ...
            BooleanPropertyView(label = stringResource(R.string.enable_nr_nsa), toggled = nrNSAEnabled) {
            ...
            BooleanPropertyView(label = stringResource(R.string.enable_nr_sa), toggled = nrSAEnabled) {
                if (VERSION.SDK_INT < VERSION_CODES.S) return@BooleanPropertyView
